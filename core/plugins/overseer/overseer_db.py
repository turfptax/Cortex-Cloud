...
class OverseerDB(CortexDB):
    ...
    # ── Category B agents (Slice 10, RETIRED in OPT-4b 2026-07-26) ──
    # The dispatch machinery (b_agents.py module, b_agent_dispatch,
    # b_agent_recent, b_agent_gc_expired, chat tool routing) is gone;
    # the merge-check prompt survives inside curator.py's structure
    # audit. b_invocation_transcripts stays as the frozen historical
    # record: Lemon's dispatch export (graded_dispatches_for_export)
    # reads latency_ms from it, and old sibling_tasks rows keep their
    # audit trail. b_agent_stats stays because check_c_graduations
    # (the C cluster, pending its own keep/kill verdict) reads it.
