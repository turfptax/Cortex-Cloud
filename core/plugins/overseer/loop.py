...
class OverseerLoop:
    ...
    def _run_one_tick_locked(self, trigger: str) -> dict:
        ...
        # frozen historical record: no new writes, no GC; Lemon's
        # dispatch export still reads latency_ms from it.)
    ...
    def backfill_notes_digest(self, *, from_day=None, to_day=None,
        ...
        try:
            ...
            # Skip days that already have a FULL digest, and only those.
            #
            # Deliberately not skipping 'mobile-notes:' days. Those were
            # written by the old phone-only step, which saw source='mobile'
            # and nothing else, so a day can carry a gist while its wearable,
            # voice and web captures were never read. Treating a partial
            # digest as done would strand those permanently, and nothing
            # would ever flag it: the day HAS a gist, so it looks processed.
            #
            # Re-digesting those days writes a second, fuller gist alongside
            # the partial one. Both are kept (owner's call, 2026-08-04): the
            # older one is not wrong, just incomplete, and rewriting history
            # costs more than the redundancy does.
            done = set()
            try:
                for row in self._db._conn.execute(
                        "SELECT period_label FROM summaries_gist WHERE "
                        "period_label LIKE 'user-notes:%'"):
